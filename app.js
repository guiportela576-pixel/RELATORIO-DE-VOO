...
function saveAll(){
  // Persistência: agora salva em IndexedDB (com backup no localStorage por segurança)
  const safeEntries = Array.isArray(entries) ? entries : [];
  try {
    // backup imediato (mantém compatibilidade / segurança)
    localStorage.setItem("flightReports", JSON.stringify(safeEntries));
    // grava no IndexedDB em background (não quebra fluxos síncronos)
    if (window.RV_IDB && typeof RV_IDB.set === "function"){
      if (saveAll._t) clearTimeout(saveAll._t);
      saveAll._t = setTimeout(() => { RV_IDB.set("flightReports", safeEntries).catch(() => {}); }, 50);
    }
  } catch (e) {}
